...
static int cuda_init(struct ra_hwdec *hw)
{
    ...
    p->is_vk = gpu != NULL;
    if (p->is_vk) {
        if (!gpu->export_caps.buf || !gpu->export_caps.sync) {
            MP_VERBOSE(hw, "CUDA hwdec with Vulkan requires exportable shared mem and sync caps\n");
            return -1;
        }
    }
#endif
                             ...
                             const struct ra_format *format, int n)
{
    ...
    const struct pl_buf *plbuf = pl_buf_create(gpu, &buf_params);
    if (!plbuf) {
        goto error;
    }
    evk->plbuf = plbuf;
...
#if !HAVE_WIN32_DESKTOP
    if (plbuf->params.handle_type == PL_HANDLE_FD) {
        mem_fd = dup(plbuf->shared_mem.handle.fd);
        if (mem_fd < 0) {
            goto error;
        }
    }
    ...
    CUDA_EXTERNAL_MEMORY_HANDLE_DESC ext_desc = {
        ...
        .handle.win32.handle = plbuf->shared_mem.handle.handle,
#else
        .type = CU_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD,
        .handle.fd = mem_fd,
#endif
        .size = plbuf->shared_mem.size,
        .flags = 0,
    };
    ...
    CUDA_EXTERNAL_MEMORY_BUFFER_DESC buf_desc = {
        .offset = plbuf->shared_mem.offset,
        .size = plbuf->params.size,
        .flags = 0,
    };
    ret = CHECK_CU(cu->cuExternalMemoryGetMappedBuffer(&evk->buf, evk->mem,
                                                       &buf_desc));
    if (ret < 0) {
        goto error;
    }
...
static void cuda_ext_vk_uninit(struct ra_hwdec_mapper *mapper, int n)
{
    ...
    if (evk) {
        ...
        if (evk->buf > 0) {
            CHECK_CU(cu->cuMemFree(evk->buf));
        }
        if (evk->plbuf) {
            pl_buf_destroy(ra_pl_get(mapper->ra), &evk->plbuf);
        }
        if (evk->ss) {
...
static int mapper_map(struct ra_hwdec_mapper *mapper)
{
    ...
    for (int n = 0; n < p->layout.num_planes; n++) {
        ...
        if (is_vk) {
            ...
            const struct pl_gpu *gpu = ra_pl_get(mapper->ra);
            pl_tex_upload(gpu, &params);
            pl_buf_export(gpu, p->evk[n].plbuf);
        }
#endif
    }
